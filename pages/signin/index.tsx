...
export default function Signin() {
  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    try {
      const signinReq = await apiService.post('auth/signin', data);
      setCookie(null, 'jwt-token', signinReq.data.token, {
        maxAge: 30 * 24 * 60 * 60,
        path: '/',
      });
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                  <label className="block text-sm">
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2.5 mb-1 text-sm leading-tight text-gray-700 border rounded shadow appearance-none focus:outline-none focus:shadow-outline"
                      type="email"
                      placeholder="Email"
                    />
                    <ErrorMessage
