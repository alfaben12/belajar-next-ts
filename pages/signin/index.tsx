...
import { SigninResponse } from '../../lib/types/signin/type';
...
export default function Signin() {
  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    try {
      ...
      setCookie(null, 'jwt-token', signinRes.token, {
        maxAge: 30 * 24 * 60 * 60,
        path: '/',
      });
      dispatch(setAuthentication(true));
      dispatch(setAuthenticationToken(signinRes.token));
      const user: UserState = {
        email: signinRes.shop.email,
        name: signinRes.shop.shop_name,
      };
